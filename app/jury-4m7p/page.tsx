...
export default function JudgePage() {
    ...
    // Reset submitted when team changes
    useEffect(() => {
    ...
    // Allow judges to re-score when admin resets score reveals
    const judgeRevealed = event?.judgeScoreRevealed;
    const prevJudgeRevealed = useRef<boolean | undefined>(undefined);
    useEffect(() => {
        if (prevJudgeRevealed.current === true && judgeRevealed === false) {
            // Admin just reset — unlock judge for re-scoring
            setSubmitted(false);
            setScore("");
            setScoreError("");
        }
        prevJudgeRevealed.current = judgeRevealed;
    }, [judgeRevealed]);
